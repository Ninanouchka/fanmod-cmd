...
// Build an EC from a maingraph
// extracted from the original nets_ctr==0 branch so threads can call it independently
...
// Thread work unit (clone graph, randomize new graph, enumerate subgraphs)
    ...
    long seed)
{
    ...
    maingraph local_maing = clone_maingraph(orig_maing);
    // thread-local RNG seed
    randlib::rand local_rand(seed); // TO CHECK
    // local copy of EC
    EdgeContainer local_EC;
    populate_EC(local_maing, local_EC, random_type, vertex_color_matters, edge_color_matters);

    res.random_time = randomize_graph(local_maing, random_type, num_exchanges, num_tries,
                                      vertex_color_matters, edge_color_matters,
                                      local_EC, res.tries, res.success, local_rand);

    long* v_ext = new long[local_maing.maxnumneighbours * G_N];
    vector<subgraph> dummy_dump;
    res.sampling_time = sampling(local_maing, v_ext, G_N, fullenumeration, prob,
                                 equiv100p, perc_number,
                                 res.inter_result, res.count_subgr,
                                 local_rand, false, dummy_dump);
    delete[] v_ext;
    free_maingraph(local_maing);
    return res;
}
...
int main(int argc, char** argv){
  ...
  desc.add_options()
      ...
      ("threads,t", po::value<int>(), "number of parallel threads for random networks generation and enumeration. default = 1") // TO CHECK DEFAULT
  ;
  ...
  if (vm.count("input") && vm.count("output")){
    ...
    // PART 1 
    // Enumerate (sample) real graph in main thread
    {
        vector<subgraph> subgraphdump;
        const bool subgraph_dumpfile = gen_dumpfile; // nets_ctr == 0
       ...
       // Write real graph results into result_graphs
    ...
    // PART 2
    // Generate and enumerate random networks in parallel threads

    // Each thread independently clones the original graph, builds its own EC, 
    // applies randomization, and enumerates subgraphs from the random network
    // Results are merged on main thread after all random networks completed enumaration
    if (num_r_nets > 0) {

        // TO CHECK TO MOVE IN RAND LOOP
        // Reestimates tree-size if the user wishes.
        // and it is not the original network being sampled
        if (reest_size && TREESMPLS > 0) {
           numtrees = est_tree_size(maing, v_extension, TREESMPLS, G_N, rand) / TREESMPLS * maing.n;
        ...
        for (int batch_start = 1; batch_start <= (int)num_r_nets;
            ...
            // Merge results into the global maps (single-threaded)
            for (int i = 0; i < batch_size; ++i) {
                const int nets_ctr = batch_start + i;
